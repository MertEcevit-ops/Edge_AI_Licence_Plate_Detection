...
/**
  * @brief  Provides a DMA RX buffer to the STM32N6 ETH HAL.
  */
void HAL_ETH_RxAllocateCallback(uint8_t **buff)
{
  *buff = eth_rx_buffer[ethRxAllocIndex];
  ethRxAllocIndex = (ethRxAllocIndex + 1U) % ETH_RX_BUFFER_CNT;
}

/**
  * @brief  Links a received DMA buffer into the HAL packet chain.
  * @note   The configured RX buffers fit a full Ethernet frame, so one
  *         buffer is enough for the current MTU.
  */
